    ...
    return (
      <div
        ...
        className={`cursor-pointer border rounded-xl p-4 shadow-sm transition-all ${
          isSelected ? "bg-blue-50 border-white" : "bg-white hover:bg-gray-50"
        }`}
      >
        <p className="font-semibold text-lg">{cls.class_name} - {cls.subject_name}</p>
        <p className="text-sm text-gray-600">Sĩ số: {cls.student_count}</p>
      </div>
    );
  })}
</div>
