...
export default function SubjectDashboard({ data }) {
  ...
  const [yearFilter, setYearFilter] = useState("2024");   // năm hiện có dữ liệu

  /* --------- Lấy toàn bộ điểm của GV bộ môn --------- */
  useEffect(() => {
    (async () => {
      try {
  ...
  useEffect(() => {
    ...
    if (byClass.length === 0) {
      console.warn("⚠️ Không tìm thấy điểm khớp bộ lọc!");
      setFilteredScores([]);
      setClassAverage(null);
      return;
    }

    /* Tính trung bình từng học sinh + TB lớp */
    const { regular_weight: rw = 1, mid_weight: mw = 2, final_weight: fw = 3 } =
      byClass[0];
    ...
    const enriched = byClass.map((s) => {
      ...
      const avg =
        ((tx1 + tx2 + tx3) * rw + gk * mw + ck * fw) / (3 * rw + mw + fw);

      return { ...s, average: +avg.toFixed(2) };
    });

    const classAvg =
      enriched.reduce((sum, s) => sum + (s.average || 0), 0) /
      enriched.length;
