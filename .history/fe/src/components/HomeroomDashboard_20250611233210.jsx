    ...
    try {
      ...
      const studentsToAdd = jsonData.map((row) => {
        ...
        if (raw) {
          ...
          if (!isNaN(raw) && typeof raw === "number") {
            const excelDate = XLSX.SSF.parse_date_code(raw);
            if (excelDate) {
              date_of_birth = `${excelDate.y}-${String(excelDate.m).padStart(2, "0")}-${String(excelDate.d).padStart(2, "0")}`;
            }
          }
          // Nếu là chuỗi
          else if (typeof raw === "string") {
            const parsed = new Date(raw.replace(/(\d{2})\/(\d{2})\/(\d{4})/, "$2/$1/$3")); // dd/mm/yyyy → mm/dd/yyyy
            if (!isNaN(parsed)) {
