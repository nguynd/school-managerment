    ...
    try {
      ...
      if (semester === "YEAR") {
        ...
        const results = await Promise.all(
          rawStudents.map(async (s) => {
            try {
              ...
              let data;
              try {
                data = await YearlyGradeAPI.get(s.id, year);
              } catch (err) {
                if (err.response?.status === 404) {
                  data = await YearlyGradeAPI.calculate(s.id, year);
                } else throw err;
              }

              return {
                ...s,
                average: data.year_score ? Number(data.year_score).toFixed(2) : "-",
        ...
        setStudents(
          list.map((s) => ({
            ...s,
            average: s.average ? Number(s.average).toFixed(2) : "-",
