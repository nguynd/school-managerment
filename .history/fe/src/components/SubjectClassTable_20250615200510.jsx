...
export default function SubjectClassTable() {
  ...
  const loadData = async () => {
    try {
      ...
      if (classScores.length > 0) {
        const subject = await getSubjectById(classScores[0].subject_id);
        setSubjectWeights({
  ...
  const calcAverage = (row) => {
    ...
    const { regular_weight: rW, mid_weight: mW, final_weight: fW } = subjectWeights;
  ...
  return (
    <div className="p-4">
      ...
      <p className="text-sm text-gray-600 mb-4">
        Trọng số: TX x{subjectWeights.regular_weight}, GK x{subjectWeights.mid_weight}, CK x{subjectWeights.final_weight}
      </p>
