...
function HomeroomDashboard({ data }) {
  ...
  const handleExcelImport = async (e) => {
    ...
    try {
      ...
      if (!("Họ tên" in firstRow) || !("Ngày sinh" in firstRow)) {
        toast.error("❌ File thiếu cột 'Họ tên' hoặc 'Ngày sinh'");
        return;
      }

      const studentsToAdd = jsonData.map((row) => ({
        name: row["Họ tên"],
        date_of_birth: XLSX.SSF.format("yyyy-mm-dd", row["Ngày sinh"]),
        class_id: data.class_id,
      }));

      const promises = studentsToAdd.map((student) =>
        HomeroomAPI.createStudent(student)
      );
