...
function ManageClassesAndStudents() {
  ...
  const [dateOfBirth, setDateOfBirth] = useState("");        // ⬅️ Ngày sinh HS
  const [gender, setGender] = useState("");                  // ⬅️ Giới tính HS
  ...
  const openModal = (type, item = null) => {
    ...
    // Nếu là modal HS
    setDateOfBirth(
      item?.date_of_birth ? item.date_of_birth.slice(0, 10) : ""
    );
    ...
    <div className="ml-64 p-4 mt-20">
      ...
      {showModal && (
        <Modal
          ...
          teacherId={teacherId}
          setTeacherId={setTeacherId}
          /* HS inputs không dùng trong form lớp */
          dateValue={dateOfBirth}
          setDateValue={setDateOfBirth}
          gender={gender}
          setGender={setGender}
    ...
    <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50">
      <div className="bg-white p-6 rounded shadow-md w-[360px]">
        <h3 className="text-lg font-semibold mb-4">
          {type.startsWith("add") ? "Thêm" : "Sửa"}{" "}
          {isClassForm ? "lớp" : "học sinh"}
        </h3>

        {/* Tên */}
        <label className="block text-sm mb-1">
          Tên {isClassForm ? "lớp" : "học sinh"}
        </label>
        <input
          value={nameValue}
        ...
        {/* Inputs HS (chỉ hiển thị với form học sinh) */}
        {!isClassForm && (
          <>
