...
const fetchStudents = async () => {
  if (viewMode === "YEAR") {
    const res = await HomeroomAPI.getStudentsByClass(data.class_id);
    const enriched = await Promise.all(
      res.map(async (s) => {
        try {
          await YearlyGradeAPI.calculate(s.id, year); // POST nếu chưa có
          const yr = await YearlyGradeAPI.get(s.id, year); // GET thông tin
          return {
            ...s,
            year_score: yr.year_score,
            year_grade: yr.year_grade,
          };
        } catch {
          return { ...s, year_score: null, year_grade: "Chưa có dữ liệu" };
        }
      })
    );
